...
pair<double, double> PurePursuit(double look_ahead_dist, double constant_velo_)
{
    ...
    double rel_x = 0.0; double rel_y = 0.0; double rel_th = 0.0;
    ...
    std::cout << "LookAheadPtX: " << LookAheadPtX << std::endl;
    std::cout << "LookAheadPtY: " << LookAheadPtY << std::endl;
...
pair<double, double> Stanley(double gainK, double constant_velo_)
{
    double ClosestPtX, ClosestPtY;
    double RefFrontPtX, RefFrontPtY;
    double steerAngle{0.0};  // [deg]
    double velo_result{0.0}; // [km/h]
    return std::make_pair(steerAngle, velo_result);
}

pair<double, double> Kanayama(double gainY, double gainOri, double velo_input)
{
    double ClosestPtX, ClosestPtY;
...
double CalculateClosestPt(int &pWIdx)
{
    ...
    {
        dist = DISTANCE(m_ref_path[i][0], m_ref_path[i][1], m_car.x, m_car.y);
        if (dist < minDist)
        {
            minDist = dist;
            pWIdx = i;
...
void GoalCheck(double check_thres)
{
    if (almost_done_flag && (DISTANCE(m_car.x, m_car.y, m_goal_pose[0], m_goal_pose[1]) < check_thres))
    {
...
void Compute()
{
    ...
    {
        CalculateClosestPt(pWIdx);
        pair<double, double> _control;
        remain_path_check(5.0);
        _control = PurePursuit(3.0, 3.0);
        // _control = Stanley(0,0, 0.0);
        // _control = Kanayama(0,0, 0,0, 0.0);
...
void CallbackLocalizationData(const std_msgs::Float32MultiArray::ConstPtr &msg)
{
    ...
    m_CarPos.pose.position.x = m_car.x;
    m_CarPos.pose.position.y = m_car.y;
